...
client = Client("chats", config.API_ID, config.API_HASH)
client.start()
        ...
        for chat in spam_list:
            ...
            try:
                with open(f'{config.DIR}{settings[1]}', 'rb') as photo:
                    ...
                    await bot.send_message(config.ADMIN, f'✅ Cообщение в {chat["title"]} было успешно отправленно.')
            except Exception as e:
                try:
                    await client.send_message(chat['id'], f"{settings[2]}\n\n{chat['text']}")
                    await bot.send_message(config.ADMIN, f'✅ Cообщение в {chat["title"]} было успешно отправленно.')
                except Exception as e:
                    await bot.send_message(config.ADMIN, f'❌ Cообщение в {chat["title"]} не было отправлено из-за ошибки: {e}')
