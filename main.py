...
async def process_start_command(m: types.Message):
    if m.chat.id == config.ADMIN:
        await bot.send_message(m.chat.id, "💾 Рассылка сообщений по группам:\n\n", reply_markup= welcome_keyboard())
    else:
...
async def input_report(m: types.Message, state: FSMContext):
    try:
        async with state.proxy() as data:
            ...
            await bot.send_message(m.chat.id, f'✅ Текст для {channel_id} был успешно обновлен.')
            await state.finish()
    except:
...
async def echo_message(m: types.Message):
    ...
    if  m.text == '👥 Доступные каналы':
        ...
        await start_spam("Accederz.tech - @MySearchX")
    elif m.text == '🛑 Остановить спам':
        db.setSpam(0)
        await bot.send_message(m.chat.id, '😊 Отправляю последнее сообщение и закругляюсь.', reply_markup=welcome_keyboard())
